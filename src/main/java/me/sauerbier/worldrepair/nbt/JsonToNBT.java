...
public class JsonToNBT{
    ...
    static JsonToNBT.Any nameValueToNBT(String key, String value) throws NBTException {
        ...
        if (value.startsWith("{")) {
            ...
            for(jsontonbt$compound = new JsonToNBT.Compound(key); value.length() > 0; value = value.substring(s.length() + 1)) {
                s = nextNameValuePair(value, true);
                if (s.length() > 0) {
                    flag = false;
                    jsontonbt$compound.tagList.add(getTagFromNameValue(s, false));
                }
